...
bank.getBalance = function (id) {
    let customer = this.transactionsDB.find(item => item.customerId == id);
    let balance = 0;
    for(let transac of customer.customerTransactions){
            balance += transac;
    }
...
bank.bankBalance = function () {
    ...
    for(let customer of this.transactionsDB){
        sum += getBalance(customer.customerId);
...
console.log(bank.transactionsDB[1]);